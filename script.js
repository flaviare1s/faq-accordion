function showAnswer() {
    const questButton1El = document.querySelector('#questButton1')
    const answer1El = document.querySelector('#answer1')
    const minus1El = document.querySelector('#minus1')
    const plus1El = document.querySelector('#plus1')

    questButton1El.addEventListener('click', () => {
        if (answer1El.style.display === 'block') {
            answer1El.style.display = 'none' 
        } else {
            answer1El.style.display = 'block' 
        }
        
        if (minus1El.style.display === 'block') {
            minus1El.style.display = 'none'
            plus1El.style.display = 'block'
        } else {
            minus1El.style.display = 'block'
            plus1El.style.display = 'none'
        }
    })

    const questButton2El = document.querySelector('#questButton2')
    const answer2El = document.querySelector('#answer2')
    const minus2El = document.querySelector('#minus2')
    const plus2El = document.querySelector('#plus2')

    questButton2El.addEventListener('click', () => {
        if (answer2El.style.display === 'block') {
            answer2El.style.display = 'none' 
        } else {
            answer2El.style.display = 'block' 
        }
        
        if (minus2El.style.display === 'block') {
            minus2El.style.display = 'none'
            plus2El.style.display = 'block'
        } else {
            minus2El.style.display = 'block'
            plus2El.style.display = 'none'
        }
    })

    const questButton3El = document.querySelector('#questButton3')
    const answer3El = document.querySelector('#answer3')
    const minus3El = document.querySelector('#minus3')
    const plus3El = document.querySelector('#plus3')

    questButton3El.addEventListener('click', () => {
        if (answer3El.style.display === 'block') {
            answer3El.style.display = 'none' 
        } else {
            answer3El.style.display = 'block' 
        }
        
        if (minus3El.style.display === 'block') {
            minus3El.style.display = 'none'
            plus3El.style.display = 'block'
        } else {
            minus3El.style.display = 'block'
            plus3El.style.display = 'none'
        }
    })

    const questButton4El = document.querySelector('#questButton4')
    const answer4El = document.querySelector('#answer4')
    const minus4El = document.querySelector('#minus4')
    const plus4El = document.querySelector('#plus4')

    questButton4El.addEventListener('click', () => {
        if (answer4El.style.display === 'block') {
            answer4El.style.display = 'none' 
        } else {
            answer4El.style.display = 'block' 
        }
        
        if (minus4El.style.display === 'block') {
            minus4El.style.display = 'none'
            plus4El.style.display = 'block'
        } else {
            minus4El.style.display = 'block'
            plus4El.style.display = 'none'
        }
    })
}

showAnswer()